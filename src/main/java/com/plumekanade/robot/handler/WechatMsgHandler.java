...
//          break;
//        }
//        case ANIME_LIST: {    // 追番列表
//          content = handleAnimeList(acgRecordService.getAllAnimations(null, 0));
//          break;
//        }
//        case ABANDON_ANIME_LIST: {    // 弃番列表
//          content = handleAnimeList(acgRecordService.getAllAnimations(null, 1));
//          break;
//        }
//        case ALL_ANIME_LIST: {    // 所有动画
//          content = handleAnimeList(acgRecordService.getAllAnimations(null, null));
//          break;
//        }
//        case ANIME_QUERY: {   // 查询某季度的动画
//          content = handleAnimeList(acgRecordService.getAllAnimations(targetName, null));
//          break;
//        }
//        case ACCOUNT: {
...
//          } else {    // 添加需要提醒的备忘录
//            memorandumService.save(new Memorandum(targetName, msgArr[2], ProjectConst.SDF.parse(msgArr[3])));
//          }
//          break;
//        }
//        case ANIME: {
...
//              acgRecordService.save(new AcgRecord(targetName, msgArr[2], msgArr[5], NumConst.ZERO, Integer.parseInt(msgArr[3]), msgArr[4]));
//            } else {
...
//              acgRecordService.save(new AcgRecord(targetName, msgArr[2], msgArr[5], NumConst.ONE, Integer.parseInt(msgArr[3]), msgArr[4]));
//            } else {
...
//          LocalDateTime expireTime = LocalDateTime.parse(msgArr[4], ProjectConst.DTF);
//          if (null != botTask) {  // 更新原来的任务
//            botTask.setMsg(msgArr[2]);
//            botTask.setStartTime(ProjectConst.SDF.parse(msgArr[3]));
//            botTask.setExpireTime(expireTime);
//            botTaskService.updateById(botTask);
//          } else {
//            botTask = new BotTask(targetName, msgArr[2], ProjectConst.SDF.parse(msgArr[3]), expireTime, "1");
...
//                .append("开始时间: ").append(ProjectConst.SDF.format(botTask.getStartTime())).append(ProjectConst.WRAP)
//                .append("结束时间: ").append(ProjectConst.DTF.format(botTask.getExpireTime())).append("\n\n");
//          }
//          content = builder.length() > 0 ? builder.substring(0, builder.length() - 2) : "没有执行任何任务...";
//          break;
//        }
//        case WEIBO_REMIND: {
//          DynamicTask.WEIBO_NUM = Integer.parseInt(null == targetName ? "0" : targetName);
//          content = "已设置微博签到连续提醒" + DynamicTask.WEIBO_NUM + "天";
//          break;
//        }
//        case ATTACH_IMG: {
//          DynamicTask.ATTACH_IMG_FLG = Boolean.parseBoolean(targetName);
//          DynamicTask.SEXY_STATE = Integer.parseInt(msgArr[2]);
//          content = "已设置" + (DynamicTask.ATTACH_IMG_FLG ? "附带" : "不附带") + "图片";
//          break;
//        }
//      }
//    } catch (Exception e) {
...
//      content = "服务器异常, 请稍后再试";
//    }
//    log.info("【微信消息】回复的消息内容: " + content);
//    return WxMpXmlOutMessage.TEXT().content(content).fromUser(wxMessage.getToUser()).toUser(wxMessage.getFromUser()).build();
    return WxMpXmlOutMessage.TEXT().content("-_-||").fromUser(wxMessage.getToUser()).toUser(wxMessage.getFromUser()).build();
  }
//
//  /**
//   * 账号相关敏感数据指令
